...
function download_comics_from_search {
	echo  "Please input an nhentai (search/tags/artists/...) website that include multi comics. Such as https://nhentai.net/search/?q=xxxxxx ."
	input_and_check_website
	echo
	#href list
...
function download_comics_form_multi_websites {
	echo "Please input nhentai comic website like https://nhentai.net/g/xxxxxx/ and press Enter. Then you can input another one and press Enter again... An empty line mean end of your input."
	while true
	do
		read webs
		if [ "$webs" = "" ]; then
